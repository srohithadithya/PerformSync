...
function ManagerReviewContent() {
  ...
  const [managerData, setManagerData] = useState({
    ...
    itemRatings: {} as Record<string, string>,
    overallRating: "",
    overallComments: "",
  ...
  const handleSectionCommentChange = (sectionId: string, value: string) => {
    setManagerData(prev => ({
      ...prev,
      sectionComments: { ...prev.sectionComments, [sectionId]: value }
    }));
  };

  const handleItemRatingChange = (itemId: string, value: string) => {
    setManagerData(prev => ({
      ...prev,
      itemRatings: { ...prev.itemRatings, [itemId]: value }
    }));
  };
    ...
    <div className="max-w-5xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
      <div className="bg-white shadow-2xl rounded-lg overflow-hidden border border-gray-100">
        ...
        <div className="px-8 py-4 bg-indigo-50 border-b border-indigo-100 flex flex-col cursor-pointer hover:bg-indigo-100 transition">
          <div className="flex justify-between items-center">
            <span className="font-medium text-indigo-900">View {employeeData?.employeeName || "Employee"}'s Submitted Self-Assessment</span>
        ...
        <form onSubmit={handleSubmit} className="px-8 py-10 space-y-12 bg-gray-50/50">
          ...
          {/* Dynamic Manager Comments Section */}
          <section className="bg-white p-8 rounded-xl shadow-sm border border-gray-100">
            <h2 className="text-xl font-semibold text-gray-800 mb-6 border-b pb-3">Manager Feedback & Ratings</h2>
            <p className="text-sm text-gray-500 mb-6">Provide feedback for each section of the employee's self-evaluation.</p>
            
            <div className="space-y-6">
              {evaluationTemplate.map(section => (
                <div key={section.id}>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Comments on: {section.title}</label>
                  <textarea 
                    rows={3} 
                    value={managerData.sectionComments[section.id] || ""} 
                    onChange={(e) => handleSectionCommentChange(section.id, e.target.value)} 
                    className="w-full border rounded-md p-3 text-sm focus:border-indigo-500 focus:ring-indigo-500 shadow-sm"
          ...
          {/* Dynamic Manager Ratings for Grid Items */}
          <section className="bg-white p-8 rounded-xl shadow-sm border border-gray-100">
            <h2 className="text-xl font-semibold text-gray-800 mb-4 border-b pb-3">Manager Rating — Specific Criteria</h2>
            <p className="text-sm text-gray-500 mb-6">Rate the employee (1-5) on the specific criteria defined in the template.</p>
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-200">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {evaluationTemplate
                    .filter(s => s.type === "rating-grid" && s.items)
                    .flatMap(s => s.items!)
                    .map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-6 py-4 text-center">
                        <input 
                          ...
                          value={managerData.itemRatings[item.id] || ""} 
                          onChange={(e) => handleItemRatingChange(item.id, e.target.value)} 
                          className="w-20 rounded-md border-gray-300 shadow-sm p-2 border sm:text-sm text-center mx-auto block" 
                        />
                      </td>
                    </tr>
                  ))}
